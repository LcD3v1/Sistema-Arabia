...
interface NovoMembroForm {
  passaporte: string; policial: string
...
function InlineCell({ value, canEdit, type = 'text', options, onSave }: InlineCellProps) {
...
function SortableRow({ membro, canEdit, onUpdate, onDelete, patentes }: SortableRowProps) {
  ...
  return (
    <tr ref={setNodeRef} style={style} className="border-b border-bdr/50 hover:bg-bdr/30 transition-colors group">
      ...
      <td className="px-3 py-2.5">
        <InlineCell field="patenteInterna" value={membro.patenteInterna} canEdit={canEdit} options={canEdit ? patentes : undefined} onSave={v => onUpdate(membro.id, { patenteInterna: v })} />
      </td>
      <td className="px-3 py-2.5">
        {canEdit ? (
          <select
...
export default function MembrosPage() {
  ...
  const canEdit = canEditArea(user, 'membros')
  ...
  async function onCreateMembro(data: NovoMembroForm) {
    try {
      await createMembro.mutateAsync(data as unknown as Omit<Membro, 'id'>)
    ...
    <div className="p-6 space-y-4">
      ...
      <ModalOverlay open={modalOpen} onClose={() => setModalOpen(false)} title="NOVO MEMBRO" maxWidth="max-w-2xl">
        <form onSubmit={handleSubmit(onCreateMembro)} className="grid grid-cols-2 gap-4">
          ...
          <div>
            <label className="font-mono text-xs text-txt2 tracking-wider block mb-1">HIERARQUIA</label>
