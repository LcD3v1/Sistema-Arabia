...
router.post('/', requireAuth, requireEdit('membros'), validateBody(membroSchema), (req: Request, res: Response): void => {
  const data = readData()
  ...
  broadcast({ resource: 'membros', action: 'created', por: req.user!.username })
  res.status(201).json(novoMembro)
})

router.put('/reorder', requireAuth, requireEdit('membros'), validateBody(reorderSchema), (req: Request, res: Response): void => {
...
router.put('/:id', requireAuth, requireEdit('membros'), validateBody(membroUpdateSchema), (req: Request, res: Response): void => {
  ...
  broadcast({ resource: 'membros', action: 'updated', por: req.user!.username })
  res.json(data.membros[idx])
})

router.delete('/:id', requireAuth, requireEdit('membros'), (req: Request, res: Response): void => {
  const id = parseInt(String(req.params.id), 10)
  if (isNaN(id)) { res.status(400).json({ error: 'ID inválido' }); return }
  ...
  broadcast({ resource: 'membros', action: 'deleted', por: req.user!.username })
  res.json({ ok: true })
})
